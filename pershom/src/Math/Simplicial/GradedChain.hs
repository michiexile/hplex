...
module Math.Simplicial.GradedChain where
...
instance (Ord a, Show a, R.Ring b, Show b) => Show (GradedChain a b) where
    show x
        ...
        where
          showTerm (d, s, c) = " + " ++ (Misc.parenthesizedShow c) ++ "*" 
                                     ++ "deg{" ++ (show d) ++ "}"
                                     ++ (S.showSansSign s)

instance (Ord a, R.Ring b) => M.AdditiveMonoid (GradedChain a b) where
    (C x) <+> (C y) = C (Misc.symmetricUnionWith (M.<+>) x y)
    nil = C Map.empty
...
homogeneous c
    | isNullChain c = False
    | otherwise = Misc.homogeneous (map Misc.first3 (toList c))
